    ...
    return (
        <div className='bg-gray-50 min-h-screen p-6'>
            <div className='flex items-center gap-4 mb-6'>
                <select onChange={type_filter} className='w-48 px-4 py-2 rounded-md border border-gray-300 focus:ring-2 focus:ring-blue-400'>
                    ...
                    <tbody className='text-gray-600'>
                        {news.slice((page - 1) * parPage, page * parPage).map((n, i) => (
                            <tr key={i} className='border-t hover:bg-gray-50'>
                                <td className='py-4 px-6'>{i + 1 + (page - 1) * parPage}</td>
                                <td className='py-4 px-6'>{n.title}</td>
        ...
        <td className='py-4 px-6'>
        {
            n.status === 'onayBekliyor' && <span onClick={() => update_status('aktif',n._id)} className='px-2 py-[2px] bg-blue-200 text-blue-800 rounded-md text-xs cursor-pointer'>{res.loader && res.id === n._id ? 'Yükleniyor..' : n.status} 
            </span>
        }
        {
            n.status === 'aktif' && <span onClick={() => update_status('deaktif',n._id)} className='px-2 py-[2px] bg-green-200 text-green-800 rounded-md text-xs cursor-pointer'>{res.loader && res.id === n._id ? 'Yükleniyor..' : n.status} 
            </span>
        }
        {
            n.status === 'deaktif' && <span onClick={() => update_status('aktif',n._id)} className='px-2 py-[2px] bg-red-200 text-red-800 rounded-md text-xs cursor-pointer'>{res.loader && res.id === n._id ? 'Yükleniyor..' : n.status} 
            </span>
        }
        </td>  
         :
        <td className='py-4 px-6'>
        {
            n.status === 'onayBekliyor' && <span className='px-2 py-[2px] bg-blue-200 text-blue-800 rounded-md text-xs'>{n.status} 
            </span>
        }
         {
            n.status === 'aktif' && <span className='px-2 py-[2px] bg-green-200 text-green-800 rounded-md text-xs'>{n.status} 
            </span>
        }
         {
            n.status === 'deaktif' && <span className='px-2 py-[2px] bg-red-200 text-red-800 rounded-md text-xs'>{n.status} 
            </span>
        }
        </td>
            ...
            <div className='flex justify-between items-center py-6'>
                <div className='flex items-center gap-4'>
                    <label className='text-sm font-semibold'>Sayfa Başına:</label>
                    <select
                        value={parPage}
                        onChange={(e) => {
                        ...
                        className='px-4 py-2 rounded-md border border-gray-300 focus:ring-2 focus:ring-blue-400'>
                ...
                <div className='flex items-center gap-4 text-sm text-gray-600'>
                    <span className='font-bold'>Sayfa {page} / {pages}</span>
